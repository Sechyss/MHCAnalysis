...
for infection= 1:size(strains, 1)
    date_new_infection = time_new_infection{new_infection_time};
    next_infection = strains{infection};
    newbetavalues = betavalues;
    changebetavalue = factor_beta{increasebeta};

    disp(['Working on ', next_infection, ' and by ', num2str(changebetavalue), 'X', ' at ', num2str(date_new_infection), ' days post infection'])
    ...
    for i = 1:size(Combination_genotypes, 1)
        ...
        for z = 1:size(proportions, 1)
            ...
            [out, t] = MultiGenotype_strain_model_2(Population, ratios, {newbetavalues, time_new, Time_simulation, new_Strain z});
    ...
    % 2 is 11, 3 is 22, and 4 is 33.
        ...
        subplot(3,2,i);
        hold on;
        scatter(proportions(:,combination(1)), peaks(:,1), 'red' , 'o', 'DisplayName','1a2a');
        scatter(proportions(:,combination(1)), peaks(:,2), 'green', 'o', 'DisplayName','1a2b');
        scatter(proportions(:,combination(1)), peaks(:,3), 'blue', 'o', 'DisplayName','1b2a');
        scatter(proportions(:,combination(1)), peaks(:,4), 'magenta', 'o', 'DisplayName','1b2b');
    ...
    saveas(gcf, ['/Users/u2176312/OneDrive - University of Warwick/Model/New Model/Betatesting/Combination_newModel_noexternalinfection/', num2str(changebetavalue), 'X_',new_Strain, '_infection_', num2str(date_new_infection),'_Genotype_ratios.pdf'])
