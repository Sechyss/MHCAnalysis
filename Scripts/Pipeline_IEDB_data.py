...
def main():
    ...
    final_df.to_csv(str(args.output) + '/' + str(args.output) + '_Cterminalmatches_location.csv', index=False)
    ...
    writer = pd.ExcelWriter(str(args.output) + '/' + str(args.output) + '_AllelePopNCBI_Workflow_SummaryTable.xlsx',
                            engine='openpyxl')
