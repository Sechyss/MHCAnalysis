...
def main():
    ...
    sequence_frame = chromosome[81 * 3 + 221323: 133 * 3 + 221323]  # Get the sequence of the region of interest

    relative_position = [x - (81 * 3 + 221323) for x in list(section_to_study['Pos'])]
    ...
    end_point = 30  # Ending point of the MHC binding region
    ...
    collector_object = []
    while end_point <= len(sequence_frame):  # Iterate through the region of interest
        kmer = indexed_sequence[starting_point: end_point]  # Get the indexed kmer
        SNP_hits = list(set(kmer.index) & set(relative_position))  # Get the SNP hits in this kmer region
        SNP_hits.sort()  # Sort the SNP hits
        if len(SNP_hits) > 0:
            dict_sliced = {x - starting_point: dict_SNPs[x] for x in SNP_hits}  # Get the dictionary of matching regions
    ...
    with open('/Users/u2176312/OneDrive - University of Warwick/'
              'CSP/SNP_kmer_rvcompl_sequence_variation_nt.fasta', 'a') as f1:
        with open('/Users/u2176312/OneDrive - University of Warwick/'
                  'CSP/SNP_kmer_rvcompl_sequence_variation_aa.fasta', 'a') as f2:
