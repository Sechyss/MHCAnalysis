...
vcf_reader = pysam.VariantFile('/Users/u2176312/OneDrive - University of Warwick/'
                               'TestVCF/SNP_INDEL_Pf3D7_03_v3.combined.filtered.vcf.gz')
...
for record in vcf_reader.fetch('Pf3D7_03_v3', 221323, 222516):
    if 'SNPEFF_GENE_NAME' in record.info.keys() and record.info['SNPEFF_GENE_NAME'] == 'CSP':
        ...
        if 'SNPEFF_CODON_CHANGE' in record.info.keys():
            ...
            codon_change = '-'
        data.append(
            [record.chrom, record.pos, record.start, record.stop, record.id, record.ref, list(record.alts), SNP_type,
             record.qual,
...
df = pd.DataFrame(data=data, columns=['Chrom', 'Pos', 'Start', 'Stop', 'ID', 'Ref', 'Alt', 'Type',
                                      'Qual', 'GQ',
                                      'Allele_frequency', 'Read_Depth', 'Amino_acid_change', 'Codon_change', 'Effect',
                                      'Impact'])

# df.to_csv('/Users/u2176312/OneDrive - University of Warwick/CSP/SNP_INDEL_Pf3D7_03_v3.csv')
# %% Addition of the MHC table to compare SNPs and MHC binding sites

df_MHC = pd.read_table('/Users/u2176312/OneDrive - University of Warwick/CSP/KenyaPopulation_mhc_i_pf3d7.txt')
...
# Warwick/CSP/SNPs_MHC_Kenya_CSF_protein_pf3d7_less10aa_kmer.pdf', dpi=300)
plt.show()
