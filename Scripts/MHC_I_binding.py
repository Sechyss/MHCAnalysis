...
from util import UnexpectedInputError, FASTASequenceInput, get_species, InputData, \
    get_peptides, MHCIAlleleData, MethodSet, get_mhc_list
...
class Prediction:
    ...
    def commandline_input(self, args):
        ...
        negative_inputs = self.check_for_negative_inputs(method, alleles, lengths,
                                                         species) if not method == "IEDB_recommended" else []
        if negative_inputs:
        ...
        if method == 'IEDB_recommended':
            header = (
            'allele', 'seq_num', 'start', 'end', 'length', 'peptide', 'method', mhc_predictor.get_score_unit(),
            'ann_ic50', 'ann_rank', 'smm_ic50', 'smm_rank', 'comblib_sidney2008_score', 'comblib_sidney2008_rank',
            'netmhcpan_el_score', 'netmhcpan_rank')
            combined_table_rows.sort(key=lambda tup: tup[7])
        elif method == 'consensus':
            header = (
            'allele', 'seq_num', 'start', 'end', 'length', 'peptide', mhc_predictor.get_score_unit(), 'ann_ic50',
            'ann_rank', 'smm_ic50', 'smm_rank', 'comblib_sidney2008_score', 'comblib_sidney2008_rank')
            combined_table_rows.sort(key=lambda tup: tup[6])
        elif method in ['netmhcpan', 'netmhcpan_el', 'netmhcpan_ba']:
            header = (
            'allele', 'seq_num', 'start', 'end', 'length', 'peptide', 'core', 'icore', mhc_predictor.get_score_unit(),
            'rank')
        ...
        if DNA_sequence_input:
            eprint(
                '# Warning: Potential DNA sequence(s) found! This tool is intended to predict for amino acid sequences. Please double check your input fasta file.')
    ...
    def add_rows_binding(self, allele, length, proteins, score_list, actual_methods_used, cutoff, value):
        for (i, (sequence, predictions)) in enumerate(zip(proteins.input_protein.as_amino_acid_text(), score_list)):
            for (k, prediction) in enumerate(predictions):
                ...
                self.row_data.append((allele, sequence_source, sequence_start, sequence_end, length, scanned_sequence,
                                      prediction, '-'.join(actual_methods_used)))
    ...
    def commandline_input_mhc(self, parser, fname):
        ...
        if DNA_sequence_input:
            eprint(
                '# Warning: Potential DNA sequences found! This tool is intended to predict for amino acid sequences. Please double check your input fasta file.')
        ...
        for seq in seq_list:
            ...
            for amino_acid in sequence.strip():
                if not amino_acid.upper() in "ACDEFGHIKLMNPQRSTVWY":
                    ...
                    sequence, amino_acid, sequence.find(amino_acid)))
                    sys.exit(1)
    ...
    def check_for_negative_inputs(self, _method_name, allele_list, length_list, species_list):
        ...
        if not any([is_user_defined_allele(a) for a in allele_list]):
            ...
            for allele, length, species in zip(allele_list, length_list, species_list):
                length_list = miad.get_allowed_peptide_lengths(
                    method_name=_method_name.replace('netmhcpan_el', 'netmhcpan'), allele_name=allele)
    ...
    def commandline_method(self):
        ...
        method_list = ['ann', 'comblib_sidney2008', 'consensus', 'IEDB_recommended', 'netmhcpan_ba', 'netmhcpan_el',
                       'smm', 'smmpmbec', 'pickpocket', 'netmhccons', 'netmhcstabpan', ]
